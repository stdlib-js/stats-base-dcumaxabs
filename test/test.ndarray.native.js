...
tape( 'main export is a function', opts, function test( t ) {
	...
	t.strictEqual( typeof dcumaxabs, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function has an arity of 7', opts, function test( t ) {
	t.strictEqual( dcumaxabs.length, 7, 'has expected arity' );
	t.end();
});

tape( 'the function calculates the cumulative maximum absolute value', opts, function test( t ) {
	var expected;
	var x;
	var y;
	var i;
	...
	dcumaxabs( x.length, x, 1, 0, y, 1, 0 );
...
tape( 'the function returns a reference to the output array', opts, function test( t ) {
	...
	out = dcumaxabs( x.length, x, 1, 0, y, 1, 0 );
...
tape( 'if provided an `N` parameter less than or equal to `0`, the function returns `y` unchanged', opts, function test( t ) {
	...
	dcumaxabs( -1, x, 1, 0, y, 1, 0 );
	t.deepEqual( y, expected, 'returns `y` unchanged' );

	dcumaxabs( 0, x, 1, 0, y, 1, 0 );
	t.deepEqual( y, expected, 'returns `y` unchanged' );
...
tape( 'the function supports negative strides', opts, function test( t ) {
	...
	dcumaxabs( 3, x, -2, x.length-1, y, -1, 2 );
...
tape( 'the function supports an `x` offset', opts, function test( t ) {
	...
	dcumaxabs( 4, x, 2, 1, y, 1, 0 );

	expected = new Float64Array( [ 1.0, 2.0, 2.0, 4.0, 0.0, 0.0, 0.0, 0.0 ] );
	t.deepEqual( y, expected, 'returns expected value' );
...
tape( 'the function supports a `y` offset', opts, function test( t ) {
	...
	dcumaxabs( 4, x, 1, 0, y, 2, 1 );

	expected = new Float64Array( [ 0.0, 2.0, 0.0, 2.0, 0.0, 2.0, 0.0, 2.0 ] );
	t.deepEqual( y, expected, 'returns expected value' );
